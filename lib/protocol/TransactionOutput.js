...
function TransactionOutput (value, script, assetId, assetQuantity, outputType) {
  ...
  this.assetId = assetId || new Buffer([]);
  this.assetQuantity = assetQuantity || 0;
...
TransactionOutput.prototype.toString = function () {
  return 'TransactionOutput(' +
    'value='         + this.value                   + ', ' +
    'script='        + ((this.script.length)  ? '0x' + this.script.toString('hex') : '') + ', ' + 
    'assetId='       + ((this.assetId.length) ? '0x' + this.assetId.toString('hex'): '') + ', ' + 
