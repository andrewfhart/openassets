      ...
      if (!foundMarkerOutput) {
        ...
        if (markerOutputPayload) {
          ...
          if (markerOutput) {
            ...
            transaction.ins.forEach(function (i, inIdx) {

              coloringEngine.getOutput(i.o, inIdx, function (err, inTx) {
                if (err) return cb(err);
                ...
                if (inputs.length == transaction.ins.length) {
                  ColoringEngine.prototype._computeAssetIds(
                    inputs,
                    outIdx,
                    transaction.outs,
                    markerOutput.assetQuantities,
                    function (err, outputsWithAssetIds) {
                      if (err) return cb(err);

                      // If valid asset ids were computed, return them
                      if (outputsWithAssetIds) return cb(null, outputsWithAssetIds);

                      // No valid asset ids in the transaction, therefore all
                      // outputs are considered uncolored
                      transaction.outs.forEach(function (o2) {
                        outputs.push(new TransactionOutput(o2.v,o2.s));
                      });

                      // Return uncolored outputs
